...
const EstudoTPEngenhariaForm = () => {
    ...
    // console.log('O valor de demandaSelecionada:', demandaSelecionada);
    ...
    const buscarProximaSequenciaNNN = async (tipoDocumento) => {
        try {
            const filtro = {
                __t: tipoDocumento  // Filtra por __t (ex: "CadernoDeNecessidades")
            };
            ...
            // Formatar com TRÊS zeros à esquerda (001, 002, ..., 999)
    ...
    const aoEnviar = async (data) => {
        ...
        try {
            ...
            const LL = "EP"
            const tipoDocumento = "EstudoTecnicoPreliminarDeEngenharia"; // hardcoded
            ...
            if (currentIdDemanda) {
                toast.info(`Código Documento BIM gerado: ${data.codigo_documento_bim}`, {
                    position: "top-right",
                    autoClose: 10000,
                });
            }
