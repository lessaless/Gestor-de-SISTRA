...
const DirinfraMultiListSelect = ({ name, label, options, registro, required, erros, setValue, watch, control, ...props }) => {
    ...
    useEffect(() => {
        setValue(name, listaValores, { shouldValidate: true, shouldDirty: true });
    }, [listaValores]);
    ...
    const aoClicarOpcoes = async (opcao) => {
        const trimmedValue = opcao.label.trim().toLowerCase(); // Remove espaços e transforma em minúsculas
        if (trimmedValue) {
            // Verifica se a palavra-chave já existe (sem considerar maiúsculas e minúsculas)
            if (!listaValores.map(valor => valor.toLowerCase()).includes(trimmedValue)) {
                setListaValores([...listaValores, trimmedValue]); // Adiciona na lista
                //setInputValue(''); // Limpa o campo
            } else alert("Palavra-chave já inserida!")
    ...
    const aoDesfocar = async (event) => {
        ...
        if ((!opcaoSelecionada?.value) && (watchInput !== '')) {
            setTermoDigitado('');
            setValue(name, listaValores, { shouldValidate: true, shouldDirty: false });
...
/* 
    ...
    const removerValor = (indexToRemove) => {
        setListaValores(listaValores.filter((_, index) => index !== indexToRemove));
    };
            ...
            <div className={classes.divInput} style={{ width: props.l ? `${props.l}px` : '65%' }}>
                ...
                */
                ...
                {listaValores.map((valor, index) => (
                    <span key={index} className={classes.valorBlock}>
                        {valor}
                        <span className={classes.removeButton} onClick={() => removerValor(index)}>
                            ×
                        </span>
                    </span>
                ))}
...
export default DirinfraMultiListSelect;