...
import DirinfraListSelectGC from '../../components/DirinfraSelect/DirinfraListSelectGC';
...
const GCDemandas = () => {
  ...
  const atualizarRefId = (colecao, objBD) => {
    console.log("atualizarRefId chamado com:", { colecao, objBD }); // ✅ Debug log
    ...
    setTemRefId(!!id);
    if (id === null) {
      console.log("Desvinculando: limpando seleção"); // ✅ Debug log
      setPecaSelecionada(null);
      carregarListaDocumentos(colecao);
      setVinculo(null);
      reset({});
  ...
  useEffect(() => {
    ...
    // ✅ ADD THIS - Check if current collection is in PEI flow
    const fluxoAtual = formulariosPlaninfra[colecao]?.fluxo;
    const isPEI = fluxoAtual === 'PEI';

    if (id_demanda) {
      if (vinculo) {
        // ✅ Only show desvincular button if NOT in PEI flow
        if (!isPEI) {
          setVinculoStatus(
            <span>
              <button
                ...
                onClick={(e) => {
                  e.preventDefault();
                  console.log("Desvincular clicked, calling callback");
          ...
          // For PEI, show nothing or a different message
          setVinculoStatus(<div></div>);
        }
      } else {
  ...
  }, [pecaSelecionada, vinculo, colecao]); // ✅ Add colecao to dependencies
          ...
          {isLoadingPeca ? (
            ...
            pecaSelecionada ? (
              ...
              >
                <SalvarProvider
                  onSaved={(savedData) => {
                    console.log("onSaved called with:", savedData); // ✅ Debug log
                    const objBD = typeof savedData === 'string' ? { _id: savedData } : savedData;
                    atualizarRefId(colecao, objBD);
                  }}
                  onDesvincular={(callback) => {
                    console.log("onDesvincular: storing callback"); // ✅ Debug log
                    desvincularRef.current = callback; // ✅ CRITICAL FIX: Store the callback
                  }}
                >
                  <div>
                    {vinculoStatus}
