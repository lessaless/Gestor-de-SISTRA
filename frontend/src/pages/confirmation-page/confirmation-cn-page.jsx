...
const ConfirmationPage = () => {
  ...
  const idCN = query.get('id_cn');
  if(!idCN) return window.location.href = '/';

  return (
    /* <div style={styles.container}> */
      <DirinfraCard style={styles.card}>
        <DirinfraCardHeader titulo="Confirmação Caderno de Necessidades" style={styles.header} />
        <DirinfraCardBody style={styles.body}>
          ...
          <h2 style={styles.title}>Caderno de Necessidades Registrado com Sucesso!</h2>
          <p style={styles.message}>
            O identificador do Caderno de Necessidades é: <span style={styles.id}>{idCN}</span>.
          </p>
          <p style={styles.note}>Por favor, anote esse código para referência futura.</p>
          <div style={styles.buttonContainer}>
            <button className='btn' style={styles.button} onClick={() => window.location.href = '/main/planinfra/listar/cns'}>
              Ir para a Lista de Cadernos de Necessidade
            </button>
          </div>
        </DirinfraCardBody>
      </DirinfraCard>
