/**
 * server.js (full, paste-ready)
 * Fixes: ERR_SERVER_ALREADY_LISTEN by starting the HTTP/HTTPS server ONLY ONCE
 * and only AFTER MongoDB connects.
 */

require("dotenv").config();

const express = require("express");
const mongoose = require("./db/connect"); // your connection module (must set mongoose.connection)
...
/**
 * Session store (only in PROD, as per your original code)
 * Note: keycloak-connect expects an express-session store-like object.
 */
if (PROD) {
    app.use(
        session({
...
// Timeout (kept from your original)
app.set("server.timeout", 100000);

/**
 * Keycloak instance (only PROD)
 */
const keycloak = PROD
    ? new Keycloak({
        store: MongoStore.create({ mongoUrl: process.env.MONGO_URI }),
 ...
 * Allowed origins
 */
const allowedOrigins = [process.env.URL, !PROD && process.env.URL_DEV]
    .map(formatarURL)
    .filter(Boolean);

/**
 * CORS (same behavior as your original)
 */
app.use(
    cors({
        origin: function (origin, callback) {
            ...
            return callback(
                new Error(`Origem não permitida: ${JSON.stringify(objErro)}`),
                false
            );
        },
        credentials: true,
...
/**
 * Extra CSRF-ish origin protection (kept)
 */
app.use((req, res, next) => {
    ...
    if (!formatted) {
        ...
        const isStaticFile =
            req.url.startsWith("/static/") || req.url.includes("/main/");
        if (!isStaticFile) {
            // optional log suppressed in your original
        }
        return next();
    }
    ...
    if (!allowedOrigins.includes(formatted)) {
        logger.error(
            `Origem não autorizada: ${formatted || "desconhecida"} - Detalhes: ${JSON.stringify(
                {
                    method: req.method,
                    url: req.url,
...
if (PROD) {
    ...
    // Serve React build without protection (as you had)
    app.use(
    ...
    // preusers without protection
    app.use("/api/preusers", userRoute);
    ...
    // React catch-all with user injection
...
/**
 * Server boot (ONLY ONCE) — after MongoDB connects
 */
let server = null;
...
db.on("error", (err) => {
    ...
    if (code === "ECONNREFUSED" || msg.includes("ECONNREFUSED")) {
        ...
        console.error(
            "Tente iniciar com ('sudo systemctl start mongod' [linux] ou 'net start mongodb' [windows: cmd, como adm]) ou verifique se a porta 27017 está aberta."
        );
    ...
    // Optional: stop process if DB fails (usually desired)
    // process.exit(1);
});

db.once("open", () => {
    const shouldDisableHttps =
    ...
    if (shouldDisableHttps) {
        ...
        server = app.listen(PORT, () => {
            const msg = `Servidor iniciado em ${PROD ? `PRODUÇÃO em ${process.env.URL}` : `DESENVOLVIMENTO na porta ${PORT}`
                } (HTTP)`;
...
const gracefulShutdown = () => {
    ...
    if (!server) {
        console.log("Servidor ainda não iniciado. Encerrando processo.");
...
// Export keycloak instance for other parts of the system
